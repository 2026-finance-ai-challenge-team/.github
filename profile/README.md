<!-- Synced from finance2026 main (3f16836); use absolute URLs for the organization profile. -->
<a id="top"></a>

<div align="center">

<img src="https://raw.githubusercontent.com/2026-finance-ai-challenge-team/finance2026/main/docs/assets/forme-hero.svg" width="100%" alt="FORM:E — Whatever the form, FORM:E it. 어떤 서류든, 준비된 형태로.">
...
<img src="https://raw.githubusercontent.com/2026-finance-ai-challenge-team/finance2026/main/docs/assets/forme-flow.svg" width="100%" alt="은행과 업무 선택 → 문서명·날짜·명의 인식 → 제출 요건 대조 → 발급 경로와 준비 체크리스트 안내">

**아직 서류가 없어도 괜찮습니다.** 업무를 선택한 뒤 **파일 없이 준비 목록 보기**를 누르면, 업로드나 OCR 없이 필요 서류·발급 기한·공식 발급 경로부터 확인할 수 있습니다.
...
<table>
  <tr>
    ...
    <td width="33%" valign="top">
      ...
      <sub>Official routes · Preparation checklist</sub>
    </td>
  </tr>
</table>
...
이 README는 **2026-09-07 제출본 `main` ([`f556615`](https://github.com/2026-finance-ai-challenge-team/finance2026/commit/f556615))**을 기준으로 작성했습니다. 코드에 구현된 경로와 공개 배포본의 검증 범위는 구분합니다.

| 영역 | 현재 코드에 있는 기능 | 범위와 조건 |
| :-- | :-- | :-- |
| 업무 선택 | 업무 목록·자연어 업무 탐색·필요서류 조회 API | 정책 DB에 등록된 은행·업무·채널을 기준으로 조회 |
| 파일 없는 준비 목록 | 필요 서류·발급 기한·제출 형태·함께 챙길 준비물 안내 | 별도 체크리스트 화면에서 조회하며 업로드·OCR 없이 이용 |
...
| 공식 경로 | 서류를 발급받는 경로와 은행의 제출 요건 출처를 구분 | 발급 경로가 확인된 경우에만 해당 링크 제공 |
| 준비 안내 ZIP API | 체크리스트·공식 경로·정리된 분석 결과 생성 | **제출본 웹에는 다운로드 버튼이 없음.** API ZIP에도 업로드 원본은 미포함 |
| 삭제 | 업로드 임시파일 정리, 분석 세션 삭제 API·화면 | 브라우저 종료만으로 세션이 자동 만료되는 기능과는 구분 |

제출본의 업무 탐색·필요서류 조회·문서 분석은 **카카오뱅크 한 업무에 고정하지 않고 정책 DB에 등록된 은행·업무를 기준**으로 동작합니다. 업무별 지원 상태는 검색 결과에서 확인합니다. 문서 제출 목록이 없는 업무는 업로드 파일을 참고용으로 확인하고 준비물·해당 은행의 공식 안내를 보여줍니다.

대표 합성 검증 시나리오는 **카카오뱅크 한도제한계좌 해제 준비**입니다. 하나은행 법인계좌, KB 금융거래 목적 증빙 및 다은행 업무 카탈로그는 [`data/seeds`](https://github.com/2026-finance-ai-challenge-team/finance2026/tree/main/data/seeds)에 있습니다. **카탈로그에 있다는 사실만으로 해당 업무의 전체 흐름이 검증됐다는 뜻은 아닙니다.** 실제 이용 범위는 적재된 정책, 공식 출처, 표본·통합 검증에 따라 달라집니다.
...
| 단계 | 하는 일 | 구현 위치 |
| :-- | :-- | :-- |
| 텍스트 확보 | PDF는 `pypdf`로 먼저 읽고, 텍스트가 부족한 경우 OCR 경로 사용 | [`extract.py`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/modules/doc_classify/extract.py) |
| 문서 분류 | 제목·발급기관·핵심 문구·레이아웃 신호를 시그니처와 대조 | [`classify.py`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/modules/doc_classify/classify.py) |
| 필드 정리 | 문서명·발급일·명시된 유효일·명의를 추출하고 근거·불확실성 보존 | [`metadata.py`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/modules/doc_classify/metadata.py) |
| 요건 연결 | 선택한 업무의 DB 정책과 문서 시그니처를 분류기에 전달 | [`analysis_service.py`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/apps/api/src/proofbridge_api/analysis_service.py) |
| 상태 판정 | 필수·대체 서류와 정책 조건을 대조하고 부족하거나 불확실한 항목 표시 | [`policy_service.py`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/apps/api/src/proofbridge_api/policy_service.py) |
| 결과 전달 | 화면용 계약으로 반환. 별도 API에서 준비 안내 ZIP 생성 | [`contracts.py`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/apps/api/src/proofbridge_api/contracts.py) · [`preparation_kit.py`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/apps/api/src/proofbridge_api/preparation_kit.py) |

파일 없이 시작하는 경로는 **업무 선택 → 필요서류 조회 → 준비 목록 화면**으로 바로 이어집니다. 이때는 문서를 전송하거나 OCR·문서 판정을 실행하지 않습니다.
...
분류 모듈의 JSON 계약은 **`schema_version: "1.0"`**입니다. 웹용 응답은 API에서 별도로 구성합니다. 정확한 필드와 샘플은 [`docs/DOC_CLASSIFY.md`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/docs/DOC_CLASSIFY.md), [`modules/doc_classify/examples`](https://github.com/2026-finance-ai-challenge-team/finance2026/tree/main/modules/doc_classify/examples), [API 계약](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/apps/api/src/proofbridge_api/contracts.py)을 참고하세요.

은행·업무별 필요서류의 기준은 **정책 DB**입니다. 로컬 `tasks/*.json`은 독립 CLI·데모용 fallback으로 유지합니다. 새로운 문서 식별자를 추가할 때는 DB와 소비 모듈의 계약을 함께 확인합니다.

별도 [`document_assessment`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/modules/document_assessment/README.md) 모듈은 하나은행 법인계좌 사례의 OCR 정규화·교차 검증·YAML 규칙 판정을 다루는 PoC입니다. 웹 API의 판정 경로와 같은 것으로 간주하지 않습니다.
...
설정 이름의 정확한 기준은 [API 설정 코드](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/apps/api/src/proofbridge_api/config.py)입니다. 자세한 외부 API 조사 자료는 [`docs/API_ONBOARDING.md`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/docs/API_ONBOARDING.md)를 참고하세요.
...
finance2026/
├── apps/
│   ├── web/                    Next.js · 준비 목록, 업로드와 결과 화면
│   └── api/                    FastAPI · 업무 조회, 분석, 세션, ZIP API
├── modules/
...
위 검사는 코드 회귀 확인용입니다. 실제 문서 인식률이나 배포 환경의 전체 성공률을 나타내지 않습니다. 평가 대상·정답표·미측정 지표는 [`eval/README.md`](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/eval/README.md)에 정리되어 있습니다.

| 더 읽기 | 내용 |
| :-- | :-- |
| [문서 분류 계약](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/docs/DOC_CLASSIFY.md) | 입력·출력 JSON, 역할 경계 |
| [API 계약](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/apps/api/src/proofbridge_api/contracts.py) | 웹에서 사용하는 요청·응답 모델 |
| [정책 시드](https://github.com/2026-finance-ai-challenge-team/finance2026/tree/main/data/seeds) | 은행·업무 요건과 공식 출처 |
| [문서 판정 PoC](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/modules/document_assessment/README.md) | 정규화 OCR과 규칙 기반 검증 |
| [평가 도구](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/eval/README.md) | 합성 데이터와 실패 유형별 검증 |
| [차별화 조사](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/docs/DIFFERENTIATION.md) | 서비스 배경과 선행 서비스 조사 |
...
- **준비 안내 ZIP API**는 안내와 정리된 결과만 담으며 제출본 웹에서는 다운로드 기능을 노출하지 않습니다. 원본 복사본을 함께 묶는 독립 CLI의 `--pack` 기능과 구분합니다.
- 민감 문서를 인쇄소에 자동 전송하거나, 공식 문서의 내용을 수정하지 않습니다.

**브랜드**

서비스 표기는 **FORM:E**, 슬로건은 **Whatever the form, FORM:E it. / 어떤 서류든, 준비된 형태로.** 입니다. 제출본의 웹 화면과 API 소개에도 FORM:E를 사용합니다. 기존 이름 **ProofBridge**는 이전 기획 자료·데모와 호환성을 유지하는 패키지명·환경변수·코드 식별자에 남아 있습니다.

**참고자료**

- [현재 공개 서비스](https://proofbridge-2026.vercel.app) — 제출본 서비스 이용 경로
...
- [기존 프로젝트 제안서 PDF](https://github.com/2026-finance-ai-challenge-team/finance2026/blob/main/output/pdf/proofbridge-project-proposal.pdf) — 이전 브랜드와 당시 기획 기준
